...
int load_image_points(cv::Size board_size, float square_size, int num_imgs, 
                       ...
                       vector<vector<Point2d> >& left_img_points, vector<vector<Point2d> >& right_img_points, 
                       vector<vector<Point3d> >& object_points, cv::Size& imageSize);


/********************************************************************××××××××××××××××××××××××××××××××××××××××
 * Function： fisheye camera stereo calinration and evaluate calibration result using 3D reconstruction
 * Date：     2018.3.6 
 * Author:    devin.gao
 * 
 * 该函数用于双目鱼眼标定和基于双目重建的标定误差评估。标定利用opencv3中的鱼眼标定，函数中已将双目标定的内部参数（M1,D1,M2,D2)
 ...
 *                  evalu_imgs_num：     评估双目鱼眼标定的图像对数量，图像的文件名字和标定图像一样，序号紧接标定图像的序号
 *                  is_show_rectified：  是否显示立体矫正的图像，1：显示，0：不显示，默认参数是不显示
 *                  is_save_result：     是否保存标定参数，1：保存，0：不保存，默认参数是保存
 * output parameters：
 *                   calib_rms_error： opencv3中双目鱼眼标定函数（cv::fisheye::stereoCalibrate）返回的rms值，表示反投影误差
 *                   calib_3D_error：  评估双目标定的三维误差
 * return value：    标定成功与否的标志 * 
 * 
 ********************************************************************/
int fisheyeStereoCalibration(int board_width, int board_height, float square_size, int calib_imgs_num,
                             ...
                             int is_show_rectified=0, int is_save_result=1)
{
    ...
    calib_rms_error = cv::fisheye::stereoCalibrate(object_points, left_img_points, right_img_points,
                                                    K1, D1, K2, D2, image_size, R, T, flags,
    ...
    //---------------------------bino_calib_fisheye------save calibration result-------------------------------------
    if (is_save_result)
    {
    ...
    //------------------------evaluate fisheye stereo calibration result---------------------------
                       ...
                       vector<vector<Point3d> >& object_points, cv::Size& imageSize)
{
    ...
    for (int i = 0; i < num_imgs; i++)
    {
        ...
        for (int k = 0; k < board_size.height; ++k)
            for (int j = 0; j < board_size.width; ++j)
                obj.push_back(Point3d(double((float)j * square_size), double((float)k * square_size), 0));
    ...
    for (int k = 0; k < imagePoints1.size(); k++)
    {
        ...
        {
            v1.push_back(Point2d((double)imagePoints1[k][j].x, (double)imagePoints1[k][j].y));
            v2.push_back(Point2d((double)imagePoints2[k][j].x, (double)imagePoints2[k][j].y));
        }
...
int main(int argc, char const *argv[])
{
    ...
    fisheyeStereoCalibration(board_width, board_height, square_size, num_imgs,
                             img_dir, leftimg_filename, rightimg_filename, 2, 
